...
class journal_print(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        ...
        self.localcontext.update({
            ...
            'get_heading' : self._get_heading,  # Trabacus
            'sum_move_debit': self._sum_move_debit, # trabacus
            'sum_move_credit': self._sum_move_credit, # trabacus            
    })
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if (data['model'] == 'ir.ui.menu'):
            ...
            context=data['form']['used_context'].update({'journal_type': data['form'].get('journal_type',False),
                                                         'account_analytic_id': data['form'].get('account_analytic_id',False)})  # Trabacus
...
# trabacus
    def _sum_move_debit(self, period_id=False, journal_id=False, move_id=False):
        if journal_id and isinstance(journal_id, int):
            journal_id = [journal_id]
        if period_id and isinstance(period_id, int):
            period_id = [period_id]
        ...
        if not (period_id and journal_id and move_id):
            return 0.0
        move_state = ['draft','posted']
        if self.target_move == 'posted':
            move_state = ['posted']
        ...
        self.cr.execute('SELECT SUM(debit) FROM account_move_line l, account_move am '
                        'WHERE l.move_id=am.id AND am.state IN %s AND l.period_id IN %s AND l.journal_id IN %s AND l.move_id IN %s' + self.query_get_clause + ' ',
                        (tuple(move_state), tuple(period_id), tuple(journal_id), tuple([self.last_move_id]) ))
        return self.cr.fetchone()[0] or 0.0
    ...
    def _sum_credit(self, period_id=False, journal_id=False):
        ...
        return self.cr.fetchone()[0] or 0.0
# trabacus
    def _sum_move_credit(self, period_id=False, journal_id=False, move_id=False):
        if journal_id and isinstance(journal_id, int):
            journal_id = [journal_id]
        if period_id and isinstance(period_id, int):
            period_id = [period_id]
    ...
    def lines(self, period_id, journal_id=False):
        ...
        if self.target_move == 'posted':
            move_state = ['posted']
        print self.query_get_clause    
#         To Do : uncomment and check why date is not coming withthe p[arent object]
#         self.cr.execute('SELECT l.id FROM account_move_line l, account_move am WHERE l.move_id=am.id AND am.state IN %s AND l.period_id=%s AND l.journal_id IN %s ' + self.query_get_clause + ' ORDER BY '+ self.sort_selection + ', l.move_id',(tuple(move_state), period_id, tuple(journal_id) ))
        self.cr.execute('SELECT l.id FROM account_move_line l, account_move am WHERE l.move_id=am.id AND am.state IN %s AND l.period_id=%s AND l.journal_id IN %s ' + self.query_get_clause + ' ORDER BY l.date, l.move_id',(tuple(move_state), period_id, tuple(journal_id) ))
        ids = map(lambda x: x[0], self.cr.fetchall())
    ...
    def _get_heading(self, data):
         a = "All Journal Book"
         if data['form']['journal_type'] == 'sale_pur' :
             a = "Sales/Purchase Book"
         elif data['form']['journal_type'] == 'cash' :
             a = "Cash Book"
         elif data['form']['journal_type'] == 'bank' :
             a = "Bank Book"
         elif data['form']['journal_type'] == 'receipt' :
             a = "Receipt Book"
         elif data['form']['journal_type'] == 'payment' :
             a = "Payment Book"
         elif data['form']['journal_type'] == 'all_other' :
             a = "All/Other Book"
             return a
         return a  
     # ~~~~~~~~~~~~~~~~
