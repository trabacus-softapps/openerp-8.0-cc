...
class account_move_line(osv.osv):
    ...
    def _query_get(self, cr, uid, obj='l', context=None):
        ...
        #   Trabacus Journal
        # ~~~~~~~~~~~~
        if context.get('journal_type') == 'receipt' or context.get('journal_type') == 'payment':
            jrnl_typ  = context['journal_type']
            query += " AND "+obj+".move_id IN (SELECT m.id FROM account_move m INNER JOIN account_voucher v on m.name = v.number AND v.type = '%s')" % (str(jrnl_typ))   # Trabacus

        if context.get('chart_account_id', False):
            child_ids = account_obj._get_children_and_consol(cr, uid, [context['chart_account_id']], context=context)
            query += ' AND '+obj+'.account_id IN (%s)' % ','.join(map(str, child_ids))
        # ~~~~~~~~~~~~
        #   Trabacus general Ledger
        # ~~~~~~~~~~~~
        if context.get('account_analytic_id', False):
             account_analytic_id  = context['account_analytic_id'][0]
             query += ' AND '+obj+'.analytic_account_id IN (%s)' % (str(account_analytic_id)) # Trabacus
