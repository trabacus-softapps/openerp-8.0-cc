...
//Anchored image
	var image = {url: '/fit_website/static/src/img/cursor-sizeall.png', 
...
{       
	var image = {url: '/fit_website/static/src/img/cursor-sizeh.png',  
...
//show the filter option on search option
    ...
    openerp.jsonRpc("/fit_website/json_maps_search/", 'call', {'center_pt':center_pt,'cuisine':get_filter_values($('#cuisine').attr('id')),'dining':get_filter_values($('#dining_style').attr('id')),'min_p':parseFloat(min_p),'max_p':parseFloat(max_p),'fav_maps':$("input[name='fav_maps']").is(':checked')})
    .then(function (data) 
	  {
...
{ 
  ...
  ({
    ...
    icon:'/fit_website/static/src/img/restaurant.png',
  });
  markers.push(marker);
