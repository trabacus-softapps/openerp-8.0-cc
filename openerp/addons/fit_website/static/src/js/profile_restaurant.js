...
function sc_included_change(sc)
{
	...
	{	
		console.log("Entered");
		$('#menu_table .gap-20').each(function()
        {   
...
$(document).ready(function () 
{
	...
	{ 
			...
			openerp.jsonRpc("/fit_website/state_json/", 'call', {'country': parseInt(country)})
            .then(function (data) 
            {	                	 
            	if(data[0].length > 0)
				{    
...
//  Restaurant Buttons 
	...
	$('.checkbox').on("click", function() 
	{
    	...
    	{
    		openerp.jsonRpc("/fit_website/address_json/", 'call')
			.then(function(data)
			{  	
			   ...
			   {	
			   		...
			   		{ 
			   			...
			   			openerp.jsonRpc("/fit_website/state_json/", 'call', {'country': countryname})
	                    .then(function (data) 
	                    {
	...
	$('.checkbox_edit').on("click", function()
	{
		...
		else
		{
		   ...
		   openerp.jsonRpc("/fit_website/existing_address_json/", 'call',{'partner_id':partner_id})
			.then(function(data)
			{
			   for(var i = 0;i<data.length;i++)
			   {
...
////////////////////////                       Adding Of New Records in Time & Allocations Tab                  /////////////////////////   
    ...
    {	
    	...
    	if(type == 'opening_hours')
    	{
	    	...
	    	'<td class="singleblock_center"><a id="db_delete" class="bookinginfolink"> <img title="Click here to delete this record" src="/fit_website/static/src/img/delete_t.png" id="img_icons"/> </a></td></tr>';
    	}
    	else
    	{
        ...
        openerp.jsonRpc("/fit_website/time/", 'call', {})
	    .then(function (data) 
	    {
		    for(var i=0; i<data[0].length; i++)
		    {	
    ...
    {	
		 ...
		 if (!cap.match(numbers))
		{
         ...
         if(covers.match(numbers))
         {
	        ...
	        openerp.jsonRpc("/fit_website/existing_alloc_record/", 'call', {'value':chkwhich,'type':type,'db_id' : db_id, 'partner_id':parseInt(venue_id),'name' : week_day, 'std_frm_id' : std_frm_id, 'std_to_id' : std_to_id, 'covers' : covers, 'cap':cap})
	        .then(function (data) 
	        {	
				...
				//alert("Applied Successfully to All !!");
		    	$("html, body").animate({
            		scrollTop: 0
        	  	}, 600);
...
////////////////////////                       Deletion of Records in Time & Allocations Tab                 /////////////////////////
     ...
     $(".alloc_data").on('click','#db_delete',function(event)
     {
	    ...
	    else
	    {
	        ...
	        if (r==true)
	        {
			    openerp.jsonRpc("/fit_website/delete_records/", 'call', {'value' : wch_table , 'db_id' : parseInt(db_id)})
		        .then(function (data) 
		        {
		         	id.remove();
		         	// document.body.scrollTop = document.documentElement.scrollTop = 0;
...
////////////////////////                       Adding Of New Records in Menu Tab                  /////////////////////////
    ...
    {	
		...
		var mm2=today.getMonth()+3;
		var yy=today.getFullYear();
	    var startdate=yy+"-"+mm1+"-"+dd;
		var enddate=yy+"-"+mm2+"-"+dd;
    	if(this.id == 'menu_add')
    	{
    		var newRow =  '<div class="row gap-20" style="margin-bottom: 20px;border: 1px solid #CCCCCC;padding: 15px;background-color: #F5F5F5;"><input class="hidden" id="db_id" value="undefined"/>'+
						  '<div class="col-sm-6 col-md-4"><div class="form-group"><label>Menu Name</label><input style="width:100%;" name="menu_name" class="form-control menu_name" title="Enter the name of the menu" value=""/></div></div>'+
		                  '<div class="col-sm-6 col-md-2"><div class="form-group"><label>Course</label>'+
						  '<select name="course" class="form-control menu_course" title="Select the menu course" style="width:100%;padding:1px;"><option  value="c1"> 1 </option><option  value="c2"> 2 </option><option  value="c3"> 3 </option><option  value="c4"> 4 </option>'+
				          '<option  value="c5"> 5 </option><option  value="c6"> 6 </option></select></div></div>'+
					      '<div class="col-sm-6 col-md-2"><div class="form-group"><label>Min. Covers</label><input style="width:100%;" name="min_covers" class="form-control min_covers" title="Enter the minimum no. of Covers" /></div></div>'+
		                  '<div class="col-sm-6 col-md-2"><div class="form-group"><label>Status</label><select style="width:100%;padding:1px;" name="active" class="form-control menu_status" title="All menus are in ACTIVE state by default. In case you do not choose to display a menu to the tour operator, change the status to INACTIVE and SAVE">'+
						  '<option value="True" > Active </option><option value="False" > Inactive </option>'+
						  '</select></div></div><div class="col-sm-6 col-md-2"><div class="form-group"><label>Price</label><input name="rest_price" class="form-control rest_price"/></div></div>'+
		 				  '<div class="col-sm-6 col-md-2"><div class="form-group"><label>From</label>' +
						  '<input name="start_date" type="text" class="start_date form-control datepicker" value="'+startdate+'"/></div>'+
					      '</div><div class="col-sm-6 col-md-2"><div class="form-group"><label>To</label><input name="end_date" type="text" class="end_date form-control datepicker" value="'+enddate+'"/>'+
				          '</div></div>'+
				          '<div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;">'+
						  '<ul class="list-unstyled info-list">' +
						  '<li><div class="checkbox"><label>' +
						  '<input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" checked="checked" name="break_fast" class="custom-radio-big type_checkbox test_break" id="break_fast"></input>'+
						  'Breakfast</label></div></li><li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="lunch" class="custom-radio-big type_checkbox" checked="checked" id="lunch"></input>'+
						  'Lunch</label></div></li></ul></div></div><div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;">'+
						  '<ul class="list-unstyled info-list"><li><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="afternoon_tea" class="type_checkbox" checked="checked" id="afternoon_tea"></input>Afternoon Tea</label></li><li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="dinner" class="custom-radio-big type_checkbox" checked="checked" id="dinner"></input>'+
			              'Dinner</label></div></li><li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER"  class="custom-radio-big type_checkbox" name="kids_menu" checked="checked" id="kids_menu"/>'+
						  'Kids Menu</label></div></li></ul></div></div>' +
						  '<div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;">'+
						  '<ul class="list-unstyled info-list"><li><div class="checkbox"><label><input type="checkbox" checked="checked" name="is_sun" class="type_checkbox" id="is_sun"></input>Sun</label></div></li>'+
						  '<li><div class="checkbox"><label><input type="checkbox" name="is_mon" class="type_checkbox" checked="checked" id="is_mon"></input>Mon</label></div></li>'+
						  '<li><div class="checkbox"><label><input type="checkbox" name="is_tue" class="type_checkbox" checked="checked" id="is_tue"></input>Tue</label></div></li></ul>'+
						  '</div></div><div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;"><ul class="list-unstyled info-list">'+
						  '<li><div class="checkbox"><label><input type="checkbox" name="is_wed" class="type_checkbox" checked="checked" id="is_wed"></input>Wed</div></li>'+
						  '<li><div class="checkbox"><label><input type="checkbox" name="is_thu" class="type_checkbox" checked="checked" id="is_thu"></input>Thu</label></div></li>'+
						  '<li><div class="checkbox"><label><input type="checkbox" name="is_fri" class="type_checkbox" checked="checked" id="is_fri"></input>Fri</label></div></li>'+
						  '<li><div class="checkbox"><label><input type="checkbox" name="is_sat" class="type_checkbox" checked="checked" id="is_sat"></input>Sat</label></div></li></ul></div></div>'+
					      '<div class="col-sm-6 col-md-8"><div class="form-group"><label>Menu Description</label>'+
						  '<textarea rows="5" style="resize:none;" name="menu_desc" class="form-control menu_desc" title="Enter the details of the menu. Please specify the inclusions in detail"></textarea>'+
						  '</div></div>'+
				          '<div class="col-sm-6 col-md-2 pull-right"><div class="form-group">'+
						  '<label class="visible-lg visible-md visible-sm visible-xs" style="visibility:hidden;">TEST</label><a id="db_delete" class="btn btn-primary bookinginfolink"> DELETE </a>'+
						  '</div></div></div>'
    	}
    	else
    	{
	    	var newRow =  '<tr class=""><form action="/capital_centric/profile/restaurant/edit_restaurant/menu" method="post"><td class="hidden"><input id="db_id" value="undefined"/></td><td class="singleblock_center menu_checkbox"><div id ="checkbox_type" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER"><span title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" id="add_label_active"> B </span><input title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" type="checkbox" name="break_fast" class="type_checkbox" id="break_fast"></input><span title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" id="add_label_active"> L </span><input title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" type="checkbox" name="lunch" class="type_checkbox" id="lunch"></input><span title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" id="add_label_active"> AT </span><input title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" type="checkbox" name="afternoon_tea" class="type_checkbox" id="afternoon_tea"></input><span title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" id="add_label_active"> D </span><input title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" type="checkbox" checked="checked" name="dinner" class="type_checkbox" id="dinner"></input><span title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" id="add_label_active"> Kids Menu </span><input title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" type="checkbox" name="kids_menu" class="type_checkbox" id="kids_menu""></input></div></td>'+
	    	...
	    	'<td class="singleblock_center"><a id="db_delete" class="bookinginfolink"> <img title="Click to delete the menu" src="/fit_website/static/src/img/delete_t.png" id="img_icons"/> </a></td></tr>';
    	}
...
////////////////////////                      Duplicating Of Records in Menu Tab                     /////////////////////////
    ...
    {	    
		...
		console.log("rest",id.find(".rest_price").val());
    	...
    	else
    	{
    		...
    		//dict['rest_price']	 	= id.find(".rest_price").val();
    		//dict['to_price']	 	= id.find(".to_price").val();
		...
		dict['start_date']       = id.find(".start_date").val();
		dict['end_date']         = id.find(".end_date").val();
    	dict['food_type']	 	 = 'meal';
    	console.log("rest dict",dict);
  	    var db_id = id.find("#db_id").val();
  		if (db_id == 'undefined')
  		{
  			db_id = 'create_1';
	    ...
	    //	'<td class="singleblock_center"><input disabled="disabled" class="form-control total_price" style="height:22px;width:100%;padding-left:5px;text-align:right;padding-right:5px;" type="text" value="'+dict['total_price']+'" title=""/></td>'+
        ...
        //else
        //{
			//var newRow =  '<div class="row gap-20" style="margin-bottom: 20px;border: 1px solid #CCCCCC;padding: 15px;background-color: #F5F5F5;">'+
			//			  '<div class="col-sm-6 col-md-4"><div class="form-group"><label>Menu Name</label><input style="width:100%;" name="form-control" class="form-control" title="Enter the name of the menu" value=""/></div></div>'+
		 //                 '<div class="col-sm-6 col-md-2"><div class="form-group"><label>Course</label>'+
			//			  '<select name="course" class="form-control" title="Select the menu course" style="width:100%;padding:1px;"><option  value="c1"> 1 </option><option  value="c2"> 2 </option><option  value="c3"> 3 </option><option  value="c4"> 4 </option>'+
			//	          '<option  value="c5"> 5 </option><option  value="c6"> 6 </option></select></div></div>'+
			//		      '<div class="col-sm-6 col-md-2"><div class="form-group"><label>Min. Covers</label><input style="width:100%;" name="min_covers" class="form-control" title="Enter the minimum no. of Covers" /></div></div>'+
		 //                 '<div class="col-sm-6 col-md-2"><div class="form-group"><label>Status</label><select style="width:100%;padding:1px;" name="active" class="form-control" title="All menus are in ACTIVE state by default. In case you do not choose to display a menu to the tour operator, change the status to INACTIVE and SAVE">'+
			//			  '<option value="True" > Active </option><option value="False" > Inactive </option>'+
			//			  '</select></div></div><div class="col-sm-6 col-md-2 hidden"><div class="form-group"><label>Restaurant Price</label>'+
		 //                 '<input style="width:100%;text-align:right;padding-right:5px;padding-left:5px;" name="rest_price" onchange="javascript:sc_included_change(this)" class="form-control rest_price" title="Enter the Restuarant Price of the menu" />'+
		 //                 '</div></div><div class="col-sm-6 col-md-2"><div class="form-group"><label>Price</label><input name="to_price" class="form-control"/></div></div>'+
		 //				  '<div class="col-sm-6 col-md-8"><div class="form-group"><label>Menu Description</label>'+
			//			  '<textarea rows="5" style="resize:none;" name="menu_desc" class="form-control menu_desc" title="Enter the details of the menu. Please specify the inclusions in detail"></textarea>'+
			//			  '</div></div><div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;">'+
			//			  '<ul class="list-unstyled info-list">' +
			//			  '<li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" checked="checked" name="break_fast" class="custom-radio-big type_checkbox test_break" id="break_fast"></input>'+
			//			  'Breakfast</label></div></li><li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="lunch" class="custom-radio-big type_checkbox" id="lunch"></input>'+
			//			  'Lunch</label></div></li></ul></div></div><div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;">'+
			//			  '<ul class="list-unstyled info-list"><li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="dinner" class="custom-radio-big type_checkbox" id="dinner"></input>'+
			//              'Dinner</label></div></li><li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER"  class="custom-radio-big type_checkbox" name="kids_menu" id="kids_menu"/>'+
			//			  'Kids Menu</label></div></li></ul></div></div><div class="col-sm-6 col-md-2 pull-right"><div class="form-group">'+
			//			  '<label class="visible-lg visible-md visible-sm visible-xs" style="visibility:hidden;">TEST</label><a id="db_delete" class="btn btn-primary bookinginfolink"> DELETE </a>'+
			//			  '</div></div></div>';
        //}
        //
    	//$(newRow).appendTo($("#menu_table"));
        //console.log('partner_id',parseInt(venue_id),db_id,dict);
        
        openerp.jsonRpc("/fit_website/existing_menu_record/", 'call', {'partner_id':parseInt(venue_id),'db_id' : db_id,'dict_menu' : dict})
        .then(function (data) 
        {	
        	...
        	//	}
	        //});
	        window.location.href = "/capital_centric/profile/restaurant/edit_restaurant/"+parseInt(venue_id)+"/menu?chk_wch=duplicate#menu_button";
			$('.loader').hide();
        });   
    });
...
////////////////////////                       Deletion Of Records in Menu Tab                  /////////////////////////
     ...
     $("#menu_table").on('click','#db_delete',function(event)
	 {
        ...
        else
        {
	        ...
	        if (r==true)
	        {
		    	openerp.jsonRpc("/fit_website/delete_records/", 'call', {'value':'menus_table','db_id' : parseInt(db_id)})
		        .then(function (data) 
		        {
		         	id.remove();
		         	 document.body.scrollTop = document.documentElement.scrollTop = 0;
...
// 	        
			...
			// {
			    ...
			    // return;
			// }
	        // openerp.jsonRpc("/fit_website/blackout_json/", 'call', {'value':chkwhich,'partner_id':venue_id,'db_id' : db_id,'date': black_date, 'black_from_time':black_from_time, 'black_to_time':black_to_time, 'name':black_desc })
	        // .then(function (data) 
	        // {
	        	// new_id = id.find('#db_id').val(data);
	        	// if(link_name == 'new' && new_id != '')
    ...
    $("#blackout_table").on('click','#db_delete',function(event)
	 {
		...
		else
		{
	        ...
	        if (r==true)
	        {
		    	openerp.jsonRpc("/fit_website/delete_records/", 'call', {'value':'black_out_table', 'db_id' : parseInt(db_id)})
		        .then(function (data) 
		        {
		         	id.remove();
		         	// document.body.scrollTop = document.documentElement.scrollTop = 0;
...
////////////////////////                       Adding Of New Records in Black-Out Tab                  /////////////////////////
    ...
    {	
    	var newRow =  '<tr class=""><td class="hidden"><input id="db_id" value="undefined"/></td><td class="singleblock_text"><input style="padding-left:5px" title="Select the blackout date" type="text" class="form-control datepicker black_inputs black_from_date" id=""></input></td>'+
    	'<td class="singleblock_text"><input style="padding-left:5px" title="Select the blackout date" type="text" class="datepicker black_inputs black_to_date form-control" id=""></input></td>'+
    	'<td class="singleblock_text"><input style="padding-left:5px" title="Enter the reason for the blackout" type="text" class="black_inputs black_desc form-control"></input></td>'+
    	'<td class="singleblock_center"><a id="db_delete" class="bookinginfolink"> <i style="     font-size: 15px; " class="fa fa-trash"></i> </a></td></tr>';
...
////////////////////////                       Adding Of New Records in Drinks Tab                  /////////////////////////
    ...
    {	
    	var newRow = '<div class="row gap-20" style="margin-bottom: 20px;border: 1px solid #CCCCCC;padding: 15px;background-color: #F5F5F5;"><input class="hidden" id="db_id" value="undefined"/>'+
					 '<div class="col-sm-6 col-md-4"><div class="form-group"><label>Name</label><input style="padding-left:5px" type="text" class="form-control black_inputs drink_name"  title=""/>'+
	    			 '</div></div><div class="col-sm-6 col-md-2"><div class="form-group"><label>Size</label><input style="padding-left:5px;text-align:right;padding-right:5px;" type="text" class="form-control black_inputs drink_size" title=""/></div>'+
					 '</div><div class="col-sm-6 col-md-2"><div class="form-group"><label>Price</label>'+
					 '<input style="padding-left:5px;text-align:right;padding-right:5px;" onchange="javascript:change_drink(this)" type="text" class="form-control black_inputs drink_price"  title=""/>'+
					 '</div></div><div class="col-sm-6 col-md-2"><div class="form-group"><label>Status</label>'+
	                 '<select style="padding:1px;" name="active" class="form-control" title=""><option value="True" > Active </option>'+
	      			 '<option value="False"> Inactive </option></select></div></div>'+
			     	 '<div class="col-sm-6 col-md-8"><div class="form-group"><label>Description</label>'+
					 '<textarea rows="4" style="" type="text" class="form-control menu_desc drink_desc" title=""></textarea>'+
					 '</div></div><div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;">'+
					 '<ul class="list-unstyled info-list"><li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" checked="checked" name="break_fast" class="custom-radio-big type_checkbox test_break" id="break_fast"></input>Breakfast</label></div></li>'+
					 '<li><div class="checkbox"><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" checked="checked" name="lunch" class="custom-radio-big type_checkbox" id="lunch"></input>'+
					 'Lunch</label></div></li></ul></div></div>'+
					 '<div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;">'+
					 '<ul class="list-unstyled info-list"><li><label><input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="afternoon_tea" class="type_checkbox" checked="checked" id="afternoon_tea"></input>'+
           			 'Afternoon Tea</label></li><li><div class="checkbox"><label>'+
					 '<input type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" checked="checked" name="dinner" class="custom-radio-big type_checkbox" id="dinner"></input>'+
     				 'Dinner</label></div></li></ul></div></div><div class="col-sm-6 col-md-2"><div class="form-group"><label class="visible-lg visible-md visible-sm visible-xs" style="visibility:hidden;">TEST</label>'+
					 '<a id="db_delete" class="btn btn-primary bookinginfolink"> DELETE </a></div></div></div>';
...
////////////////////////                       Adding Of New Records in Events Menu Tab                  /////////////////////////
    ...
    {	
    	var newRow = '<div class="row gap-20" style="margin-bottom: 20px;border: 1px solid #CCCCCC;padding: 15px;background-color: #F5F5F5;"><input class="hidden" value="undefined" id="db_id"/>'+
					 '<div class="col-sm-6 col-md-4"><div class="form-group"><label>Name</label><input style="padding-left:5px" type="text" class="form-control black_inputs drink_name" title=""/>'+
		             '</div></div><div class="col-sm-6 col-md-2"><div class="form-group"><label>Status</label><select id="sc_included_drink" class="form-control sc_included_drink" style="" onchange="javascript:change_drink(this)">'+
		             '<option value="True" > Yes </option>'+
					 '<option value="False"> No </option>'+
					 '</select></div></div><div class="col-sm-6 col-md-2"><div class="form-group"><label>Price</label>'+
		          	 '<input style="padding-left:5px;text-align:right;padding-right:5px;" onchange="javascript:change_drink(this)" type="text" class="form-control black_inputs drink_price" />'+
					 '</div></div><div class="col-sm-6 col-md-8"><div class="form-group"><label>Description</label>'+
					 '<textarea rows="5" style="" type="text" class="form-control menu_desc drink_desc" title=""></textarea>'+
	                 '</div></div><div class="col-sm-6 col-md-2"><div class="form-group" style="margin-top: 20px;"><ul class="list-unstyled info-list">'+
					 '<li><div class="checkbox"><label><input checked="checked" type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the Event Menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER"  name="break_fast" class="custom-radio-big type_checkbox" id="break_fast"/>'+
					 'Breakfast</label></div></li><li><div class="checkbox"><label><input checked="checked" type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the Event Menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="lunch" class="custom-radio-big type_checkbox"  id="lunch"/>'+
					 'Lunch</label></div></li></ul></div></div><div class="col-sm-6 col-md-2">'+
				     '<div class="form-group" style="margin-top: 20px;"><ul class="list-unstyled info-list"><li><label><input checked="checked" type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="afternoon_tea" class="type_checkbox" id="afternoon_tea"></input>Afternoon Tea</label></li>' +
					 '<li><div class="checkbox"><label><input type="checkbox" checked="checked" title="Check the appropriate boxes to indicate the timing of availability of the Event Menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="dinner" class="custom-radio-big type_checkbox"  id="dinner"/>'+
					 'Dinner</label></div></li><li><div class="checkbox"><label><input checked="checked" type="checkbox" title="Check the appropriate boxes to indicate the timing of availability of the Event Menu. B - BREAKFAST, L - LUNCH, AT - AFTERNOON TEA, D - DINNER" name="kids_menu" class="custom-radio-big type_checkbox"  id="kids_menu"/>'+
					 'Kids Menu</label></div></li></ul></div></div><div class="col-sm-6 col-md-2 pull-right"><div class="form-group">'+
				     '<label class="visible-lg visible-md visible-sm visible-xs" style="visibility:hidden;">TEST</label>'+
					 '<a id="db_delete" class="btn btn-primary bookinginfolink"> DELETE </a></div></div></div>';
    ...
    $("#events_table").on('click','#db_delete',function(event)
	 {
		...
		else
		{
	        ...
	        if (r==true)
	        {
		    	openerp.jsonRpc("/fit_website/delete_records/", 'call', {'value':'events_table', 'db_id' : parseInt(db_id)})
		        .then(function (data) 
		        {
		         	id.remove();
		         	// document.body.scrollTop = document.documentElement.scrollTop = 0;
...
////////////////////////                       Adding Of New Records in Events Menu Tab                  /////////////////////////
    ...
    {	
    	var ul = ''; 
    	var newRow = '<tr class="last_tr"><td class="hidden"><input class="hidden" id="db_id" value="undefined"/></td><td class="singleblock_text"><input style="padding:5px;width:100%;" class="form-control black_inputs drink_name" title=""></input></td>'+
    	...
    	'<td class="singleblock_text" id="multi"><select name="event_menu_name" id="event_menu" class="form-control input_val event_menu_new" title="" multiple="multiple" onchange="javascript:get_eventvalues(this)"></select><textarea class="hidden" name="e_menu_selected" id="event_menu_ids"></textarea></td>'+
    	...
    	'<td class="singleblock_center"><a id="db_delete" class="bookinginfolink"> <img title="Click to delete this record" src="/fit_website/static/src/img/delete_t.png" id="img_icons"/> </a></td></tr>';   
		...
		openerp.jsonRpc("/fit_website/events_menu/", 'call', {'partner_id':parseInt(venue_id)})
	    .then(function (data) 
	    {   
		     for(var i=0; i<data[0].length; i++)
	    	 {	
...
////////////////////////                       Deletion Of Records in Private Rooms Tab                 /////////////////////////
    ...
    $("#pvt_room_table").on('click','#db_delete',function(event)
	{
		...
		else
		{
	        ...
	        if (r==true)
	        {
		    	openerp.jsonRpc("/fit_website/delete_records/", 'call', {'value':'private_room_table', 'db_id' : parseInt(db_id)})
		        .then(function (data) 
		        {
		         	id.remove();
		         	// document.body.scrollTop = document.documentElement.scrollTop = 0;
...
//  For Image Tab
   ...
   $(".restaurant_img").on("click", function()
   {
	   console.log("entered");
   ...
   $("input[name='image_name']").on("change", function()
   {
	...
	{ 
		...
		openerp.jsonRpc("/fit_website/image_desc/", 'call', {'partner_id':parseInt(venue_id), 'dict' : a})
        .then(function (data)
        {
        	//$("html, body").animate({
            	//	scrollTop: 0
...
// For Deletion of The Restaurants
     $("#res_table").on('click','#db_delete',function(event)
     {
        ...
        if (r==true)
        {
	    	openerp.jsonRpc("/fit_website/delete_records/", 'call', {'value':'partner_table', 'db_id' : parseInt(db_id)})
	        .then(function (data) 
	        {
        	    $("html, body").animate({
            		scrollTop: 0
   ...
   // Apply To all link in Pricing Table
	$('.apply_link').click(function()
	{
		    ...
		    openerp.jsonRpc("/fit_website/pricing_json/", 'call', {'dict_pricing':dict,'menu_id':parseInt(menu),'chkwhich':'apply_all','sc_included':sc_included,'service_charge':service_charge, 'sc_disc':sc_disc})
		   .then(function (data)
		   { 
	         	 //$("html, body").animate({
            		//scrollTop: 0
...
//save for pricing table
        if(button_clk == 'pri_save')
        {
	        ...
	        for (i=0;i < m.length; i++)
	 		...
	 		openerp.jsonRpc("/fit_website/pricing_json/", 'call', {'dict_pricing':vals,'menu_id':parseInt(menu), 'chkwhich':'save','sc_included':sc_included,'service_charge':service_charge, 'sc_disc':sc_disc})
		  	.then(function (data)
		  	{	
		  	   // $("html, body").animate({
            		//scrollTop: 0
...
//Save for Time & Allocations table
	    ...
	    if (button_clk == 'bf_save' || button_clk == 'lun_save' || button_clk == 'at_save' || button_clk == 'din_save' || button_clk == 'pr_save')
	    {
			...
			{
				openerp.jsonRpc("/fit_website/save_time_rec/", 'call', {'dict_time':vals,'type':alloc_type,'venue_id':parseInt(venue_id),'create_count':count,'chkwhich':'allocations'})
			  	.then(function (data)
...
//Save for black-out table
	    if(button_clk == 'bo_save')
        {
			...
			{
				openerp.jsonRpc("/fit_website/save_time_rec/", 'call', {'dict_time':vals,'type':'','venue_id':parseInt(venue_id),'create_count':count,'chkwhich':'black_out'})
			  	.then(function (data)
			  	{	console.log('data',data);
...
//Save for drinks table
        ...
        {
	    	$('#drinks_table .gap-20').each(function()
	        {   var dict={};
				...
				dict['parent_res']	 	=	$(this).find("#drink_restaurant_list").val();
	        	...
	        	if ($(this).find(".menu_status").val() == 'False')
		  		...
		  		var db_id = $(this).closest('.gap-20').find("#db_id").val();
				...
				if((dict['service_charge'] == '' || dict['service_charge'] <= 0.00) && dict['sc_included'] == 'False')
				{
		  		...
		  		{
		  			db_id = 'create_'+(count++);
		  			$(this).closest('.gap-20').find("#db_id").val(db_id);
		  		}
		  		vals[db_id] = dict;
			});
			var check_value = document.getElementById('check_value').value;
			...
			{
				openerp.jsonRpc("/fit_website/save_time_rec/", 'call', {'dict_time':vals,'type':'','venue_id':parseInt(venue_id),'create_count':count,'chkwhich':'drinks'})
			  	.then(function (data)
			  	{	
				  	$('#drinks_table .gap-20').each(function()
			        {	
...
//Save for Private Room table
	    if(button_clk == 'pvt_room_save')
        {
	        ...
	        {   var dict={};         
	        	...
	        	dict['others'] 	   	   = $(thlleis).find(".others").val();
		  		var db_id = $(this).find("#db_id").val();
		  		if(dict['name'] == '')
		  		{
			...
			{
				openerp.jsonRpc("/fit_website/save_time_rec/", 'call', {'dict_time':vals,'type':'','venue_id':parseInt(venue_id),'create_count':count,'chkwhich':'private_room'})
			  	.then(function (data)
			  	{	console.log('data',data);
				  	$('table#pvt_room_table tr:gt(0)').each(function()
			        {	
...
//Save for Events Menu table
        ...
        {	
        	$("#dvLoading").show();
	    	$('#events_menu_table .gap-20').each(function()
	        {   var dict={};         
	        	dict['break_fast']    	 =  $(this).find("#break_fast").is(':checked');;
	        	dict['lunch'] 		  	 =  $(this).find("#lunch").is(':checked');
                dict['afternoon_tea'] 	 =  $(this).find("#afternoon_tea").is(':checked');
			...
			{
				openerp.jsonRpc("/fit_website/save_time_rec/", 'call', {'dict_time':vals,'type':'','venue_id':parseInt(venue_id),'create_count':count,'chkwhich':'events_menu'})
			  	.then(function (data)
			  	{	console.log('data',data);
				  	$('#events_menu_table .gap-20').each(function()
			        {	
...
//Save for Events table
	    if(button_clk == 'event_save')
        {
			...
			{
				openerp.jsonRpc("/fit_website/save_time_rec/", 'call', {'dict_time':vals,'type':'','venue_id':parseInt(venue_id),'create_count':count,'chkwhich':'events'})
			  	.then(function (data)
			  	{	console.log('data',data);
				  	$('table#events_table tr:gt(0)').each(function()
			        {	
...
//save for menus table
	    if(button_clk == 'menu_save' || button_clk == 'menu_save_group')
        {
	        ...
	        {   var dict={};         
                ...
                dict['dinner'] 		  = $(this).find("#dinner").is(':checked');
                dict['kids_menu'] 	  = $(this).find("#kids_menu").is(':checked');
				dict['is_sun'] 	      = $(this).find("#is_sun").is(':checked');
				dict['is_mon'] 	      = $(this).find("#is_mon").is(':checked');
				dict['is_tue'] 	      = $(this).find("#is_tue").is(':checked');
				dict['is_wed'] 	      = $(this).find("#is_wed").is(':checked');
				dict['is_thu'] 	      = $(this).find("#is_thu").is(':checked');
				dict['is_fri'] 	      = $(this).find("#is_fri").is(':checked');
				dict['is_sat'] 	      = $(this).find("#is_sat").is(':checked');
                dict['course']		  = $(this).find(".menu_course").val();
                dict['min_covers']    = $(this).find(".min_covers").val();
				dict['start_date']    = $(this).find(".start_date").val();
				dict['end_date']      = $(this).find(".end_date").val();
                dict['food_type']	  = 'meal';
                console.log("bbbb",dict);
			...
			{
				openerp.jsonRpc("/fit_website/save_time_rec/", 'call', {'dict_time':vals,'type':'','venue_id':parseInt(venue_id),'create_count':count,'chkwhich':'menu'})
			  	.then(function (data)
			  	{	
				  	$('#menu_table .gap-20').each(function()
			        {	
					...
					document.getElementById('save_prompt').innerHTML="Saved Successfully !!";
					$("#save_prompt").stop(true,true).show().fadeOut(5000);
			        window.location.href = "/capital_centric/profile/restaurant/edit_restaurant/"+parseInt(venue_id)+"/menu?chk_wch=save#menu_button";
			 	});
		    }
		}
	});
...
function showselection()
{
	 var type = $(this).attr('id');
	 console.log(type);
	 var opt = document.getElementById("con_contact");
...
function show_inv_selection()
{
	var type = $(this).attr('id');
	console.log(type);
	var opt = document.getElementById("inv_contact");
	...
	console.log('selValue',selValue);
	document.getElementById("txt_inv_Editions").innerHTML = selValue
}