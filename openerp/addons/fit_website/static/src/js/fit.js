...
function amend_and_cancel(chk_wch,p)
{
	...
	openerp.jsonRpc("/fit_website/menu_and_covers_for_bookings/", 'call', {'lead_id' : parseInt(lead_id)})    	
	.then(function (data)
	{	
        //fader = document.getElementById('login_fader');
	    //var login_box = document.getElementById('div3');
...
function continue_button(chk_wch)
{
	...
	console.log("info_dict",menu_list,$("#send_confirmation_fit").is(':checked'),info_dict);
	openerp.jsonRpc("/fit_website/menu_covers_save/", 'call', {'menu_list' : menu_list,'send_confirmation' : $("#send_confirmation_fit").is(':checked'),'info_dict' : info_dict})
    .then(function (data)
    {
...
{	dict = {};
	...
	{	
		...
		dict['spcl_req']     = $("#spcl_req").val();
     	i++;		
     	console.log(dict);
     	openerp.jsonRpc("/fit_website/save_input_my_cart/", 'call', {'value' : dict, 'db_id' : parseInt(info_id)});
     	$(".guest_input").prop("disabled",true);
