...
{    
      ...
      openerp.jsonRpc("/fit_website/cc_menus_table/", 'call', {'db_id':parseInt(db_id), 'price': new_value, 'guest': guest,'menu_ids':menu})
      .then(function (data) 
      {	
      		if(e.name == 'guest' || e.name == 'eguest')
      		{
...
function onchange_meal_date(e)
{
    ...
    openerp.jsonRpc("/fit_website/cc_menus_table/", 'call', {'db_id':event_id, 'price': 0, 'guest': 0, 'menu_ids':menu, 'frmwch': frmwch, 'e_date':edate})
      .then(function (data) 
      {
	      	if (data && data[1])
	      	{
...
// On click of "Make Another Booking" & "Complete Booking" on mycart page	
	...
	{	
	    ...
	    var vals = {};
     	$('.order-inner-content .order-delivery-address').each(function()
        {   var dict = {};
			dict['booker_name']       = $(this).find(".booker_name").val();
            ...
            var db_id  				  = $(this).find(".db_id_info").val();
           	...
           	if(dict['booker_name'] == '')
	  		{	
	  			$(this).find(".booker_name").focus().css('border','1px solid #ED1E6B');
			...
			if(dict['cust_ref'] == '')
	  		{
	  			$(this).find(".cust_ref").focus().css('border','1px solid #ED1E6B');
	  			// $(this).find("tr#alert_message").show();
	  			// $(this).find("#to_ref_alert").innerHTML = "Please Enter ";
	  			// $(this).document.getElementById('to_ref_alert').innerHTML="Please Enter an Email!!";
	  			document.getElementById("check_value").value = 1;
	  		...
	  		if(dict['title'] == '')
	  		{	
	  			$(this).find(".title").focus().css('border','1px solid #ED1E6B');
	  			// $(this).find("tr#alert_message").show();
	  			document.getElementById("check_value").value = 1;
	  			return false;
	  		}
	  		...
	  		{	
	  			$(this).find(".partner_name").focus().css('border','1px solid #ED1E6B');
	  			// $(this).find("tr#alert_message").show();
	  			document.getElementById("check_value").value = 1;
	  			return false;
	  		}
	  		...
	  		{	
	  			$(this).find(".contact_name").focus().css('border','1px solid #ED1E6B');
	  			// $(this).find("tr#alert_message").show();
	  			document.getElementById("check_value").value = 1;
	  			return false;
	  		}
	  		...
	  		{	
	  			$(this).find(".mobile").focus().css('border','1px solid #ED1E6B');
	  			// $(this).find("tr#alert_message").show();
	  			document.getElementById("check_value").value = 1;
	  			return false;
	  		}
	  		...
	  		{	
	  			$(this).find(".email_from").focus().css('border','1px solid #ED1E6B');
	  			// $(this).find("tr#alert_message").show();
	  			document.getElementById("check_value").value = 1;
	  			return false;
	  		}
	  		if(dict['terms'] == false)
	  		{	
	  			$(this).find("#terms").focus().css('border','1px solid #ED1E6B');
	  			// $(this).find("tr#alert_message").show();
	  			document.getElementById("check_value").value = 1;
	  			return false;
	  		}
		...
		else
		{
			if(button_id == 'complete')
			{
				if($('.paymentType').val() == '')
	    		{
	    			$(".paymentType").focus().css('border','1px solid #ED1E6B');
		  			// document.getElementById("check_value").value = 1;
		  			return false;
	    		}
...
// On blur of input fields in mycart page
   ...
   {	
		...
		var value = this.value;
	 	var db_id = $(this).closest(".order-delivery-address");
		...
		openerp.jsonRpc("/fit_website/save_input_my_cart/", 'call', {'value' : dict, 'db_id' : parseInt(db_id.find(".db_id_info").val())});
   });

// On click of "Remove from Cart" in mycart page	
	$('.remove_from_cart').on('click',function()
	{
		//var id = $(this).parent().closest("tr");
  		var db_id = $(this).closest('.order-process-left-side').find(".db_id_info").val();
  		var r=confirm("Are you sure you want to remove this from your cart ?");
        if (r==true)
        {
		    openerp.jsonRpc("/fit_website/delete_records/", 'call', {'value' : 'lead_table', 'db_id' : parseInt(db_id)})
	        .then(function (data) 
	        {	
	         	//db_id.remove();
...
// On click of close icon in "Select Time" Block
    ...
    // 				var partner_id = $("#partner_id").val();
 	//				var menu = [];
		...
		//			            });
		//			});
    //            	openerp.jsonRpc("/fit_website/onclick_date/", 'call', {'price_type':calEvent.price_type, 'type':type, 'partner_id':parseInt(partner_id), 'date':calEvent.start, 'min_time':calEvent.min_time, 'max_time':calEvent.max_time,'menu_ids':menu})
    //  				.then(function (data)
    //  				{
    ...
    //                        	}
	 ...
	 //
		...
		//						{
		//							kids_menu_td = '<td style="width:5%;"><img id="" class="" src="/fit_website/static/src/img/kidsmenu.png" alt="Kids Menu Icon" style="height:20px;" title="Kids Menu"/></td>';
		//						}
		//
		//						tr_style = '';
		//						if (data[0][i]['to_mu'] > 0)
...
function onchange_time(e)  
 {  
     ...
     if(type != '')
     {
 		...
 		openerp.jsonRpc("/fit_website/on_change_type/", 'call', {'partner_id':parseInt(partner_id),'type':type,'date':new Date()})
	     .then(function (data)
	     {	
	 ...
	 if (id == 'time')
	 {	
		 openerp.jsonRpc("/fit_website/blackout_validation/", 'call', {'partner_id':parseInt(partner_id), 'time':parseInt(time),'date':date,'type':type,'chkwhich':'time'})
	     .then(function (data) 
		  {
		    if (data == false)
		    {	
	 ...
	 {	 	
	 	...
	 	{
	 	   $('#time').empty();
	 	   openerp.jsonRpc("/fit_website/blackout_validation/", 'call', {'partner_id':parseInt(partner_id), 'time':parseInt(time),'date':date, 'type': type,'chkwhich':'all'})
	 	   .then(function (data) 
		  {
