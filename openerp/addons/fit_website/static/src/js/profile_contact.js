$(document).ready(function () 
{
				...
				.then(function(data)
				{
				   for(var i = 0;i<data.length;i++)
				   {
				   		...
				   		{ 
				   			...
				   			var location = data[i];
				   			$('#con_location').empty().append('<option value="">Select Location ..</option>');
				   			openerp.jsonRpc("/fit_website/location_json/", 'call', {'city': cityname, 'value':'city_clicked'})
		                    .then(function (data) 
		                    {
						    	for(var i=0; i<data[0].length; i++)
						    	{
    ...
    $('.button').click(function()
    {
    	...
    	if($(this).attr('id') == 'activate_user')
    	{
    		...
    		if($('input[name="type"]').val() == 'restaurant' && (role == 'client_user' || role == 'venue_user'))
	    	{
		    	...
		    	{
			    		$('#res_con_contact').focus().css('border','1px solid #ED1E6B');
			    		$('#plz_select_res').show();
			    		return false;
		    	}
	    	}
	...
	$("#res_con_contact").multiselect({
		noneSelectedText: "Select Restaurants",
		selectedText : "# Restaurant(s) Selected",
		click: function(e)
		{
			if($(this).multiselect("widget").find(":checked").length > 0 )
			{
				$('#plz_select_res').hide();
			}
			else
	...
	$('.ui-multiselect-menu').outerWidth($('.ui-multiselect').outerWidth());
	$('.ui-multiselect-checkboxes label input').css('margin-right','1%');
    ...
    // {
	    // openerp.jsonRpc("/fit_website/check_email/", 'call', {'value': $(this).val(), 'db_id' : parseInt($("#partner_id").val())})
	 	// .then(function(data)
	 	// {	
	 		// if(data == true)
	 		// {    
