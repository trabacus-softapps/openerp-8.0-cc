...
;(function($, window, document) {
  ...
  var FJS = function(records, container, options) {
    ...
    this.criterias = [];
    this._index = (records != undefined && records.length > 0)? records[0]['unique_id'] : 0;//added by suraj for filter purpose
    ...
    if(criteria.all && vals.indexOf(criteria.all) > -1){
      ...
      if (criteria.field == 'stop_qty')
      {
        if (vals.length == 0)
        {
          ...
          var sp_arr = vals;
          vals = [sp_arr[0].split(criteria.delimiter || '-')[0]];
          vals.push(sp_arr[(sp_arr.length) - 1].split(criteria.delimiter || '-')[0]);
          vals = [Math.min.apply(Math, vals), Math.max.apply(Math, vals)];
        }
      }
      else {
  ...
  F.show = function(result, type){
    ...
    $('.fjs_item').css('display','none');

    for(i; i < l; i++){
      $('#fjs_' + result[i]._fid).css('display','block');
    }
  ...
  F.setStreaming = function(opts){
    ...
    if(opts.data_url || opts.json_data){
  ...
  var fetchData = function(){
    ...
    //Added by suraj for loading the records from JSON Dict which stored in session
    if(opts.json_data){

      $.when(get_json_dict(params)).done(function (records) {
          if (params.limit != null && (!records || !records.length)) {
              self.stopStreaming();
          } else {
              self.setStreamInterval();
              self.addRecords(records, false);
          }
      }).fail(function (e){
            self.stopStreaming();
        });
  ...
  P.render = function(){
    ...
    var page_html = this.paginationTmpl(pages).replace('<><>',' ')
    this.$container.html(page_html.replace(/\<>/g, ' '))
  };
  ...
  views['pagination'] = '<nav><ul class="pagination">'+
      '<% if(currentPage > 1){%>'+
      '<li><a href="#" data-page="first" aria-label="First"><span aria-hidden="true">First</span></a></li>'+
      '<li><a href="#" data-page="prev" aria-label="Previous"><span aria-hidden="true">&larr; Previous</span></a></li>'+
      '<%}%>'+
      '<%if( currentPage < totalPages ){%>'+
      '<li><a href="#" data-page="next" aria-label="Next"><span aria-hidden="true">Next &rarr;</span></a></li>'+
      '<li><a href="#" data-page="last" aria-label="Last"><span aria-hidden="true">Last</span></a></li>'+
      '<%}%>' +
      '</ul></nav>';
  views['per_page'] = '<select size="1" name="per_page" data-perpage="true" class="per-page">' +
