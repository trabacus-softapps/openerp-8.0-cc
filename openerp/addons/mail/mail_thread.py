...
class mail_thread(osv.AbstractModel):
    ...
    def message_subscribe(self, cr, uid, ids, partner_ids, subtype_ids=None, context=None):
        ...
        for id in ids:
            ...
            #Suraj:line added to remove followers who are not users
            if new_pids:
               cr.execute("select partner_id from res_users where role in ('admin','bo') and partner_id in %s",(tuple(new_pids),))
               new_pids = [x[0] for x in cr.fetchall()]
