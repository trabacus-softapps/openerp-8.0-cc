...
class CommonReportHeaderWebkit(common_report_header):
    ...
    #Added by Suraj (Function for Create Date Filter)
    def _get_move_ids_from_createdates(self, account_id, date_start, date_stop, target_move, mode='include_opening'):
        # TODO imporve perfomance by setting opening period as a property
        move_line_obj = self.pool.get('account.move.line')
        search_period = []

        if date_start:
            search_period = [('create_date', '>=', date_start),
                             ('account_id', '=', account_id)]
        if date_stop:
            search_period = [('create_date', '<=', date_stop),
                             ('account_id', '=', account_id)]
    ...
    def get_move_lines_ids(self, account_id, main_filter, start, stop, target_move, mode='include_opening'):
        ...
        elif main_filter == 'filter_create_date':#added by Suraj (new filter for capitalcentric)
            return  self._get_move_ids_from_createdates(account_id, start, stop, target_move)
        else:
