...
class Website(openerp.addons.web.controllers.main.Home):
    ...
    def page(self, page, **opt):
        ...
        _logger.error('WORLD PAY POST: %s',request.httprequest.session.get('referral_link'))
#         request.session.authenticate(request.db, user.login, user.password)      
        ...
        if user.is_wlabeling and 'redirect' not in opt:
           return request.redirect('/capital_centric/fit/grid_view')
...
def get_price(cr,day,pr,operator_id):    
    ...
    if result and operator.comision_type == 'markup':
...
#-------------------------------
      ...
      def restaurant_login(self,*arg, **post ):
        ...
        return request.website.render("website_capitalcentric.res_layout",{'user':user})  
...
#                tracking_obj.create(cr, uid, {'name':user.id, 'login':time.strftime("%Y-%m-%d %H:%M:%S")})
               ...
               if post.get('login_type') == 'venue':
                    if(user.partner_id.accept_tc == True):
                        ...
                        return request.website.render("website_capitalcentric.tc", values)
            ...
            if post.get('login_type') != usr_role[user.role]:
                ...
                if post.get('login_type') == 'venue':
                        ...
                        return request.website.render("website_capitalcentric.res_layout", values) 
                else:    
                        values['error'] = "Wrong Login/Password"
...
#SIGN UP LINK
      ...
      def signup(self, path,**post ):
          ...
          return request.website.render("website_capitalcentric.signup_page", values)
      

#SIGN UP FORM
      @http.route(['/website_capitalcentric/sign_up_details/'], type='http' ,auth="public" ,website=True ,multilang=True)
      def sign_up_details(self, **post ):
          ...
          if post.get('type') not in ('venue','operator'):
             ...
             err_msg = 'Thank You for signing up. Please click on the continue button to proceed further.'
          ...
          return request.website.render("website_capitalcentric.signup_page",{'req':'requested','error':err_msg,'user':user,'post':post})
      
      
#JSON FOR CHECKING E-MAIL IS EXISTING OR NOT IN DATABASE
      @http.route(['/website_capitalcentric/check_email/'], type='json', auth="public", website=True, multilang=True)
...
# 'Cancellation' link in Operator 'BOOKINGS'    
      @http.route(['/website_capitalcentric/menu_and_covers_for_bookings/'], type='json', auth="public", website=True)
      def menu_and_covers_for_bookings(self,lead_id,json=None):
          ...
          if lead.date_requested < today:
             return request.website._render("website_capitalcentric.menu_and_covers", {'not_allowed': 'not_allowed','menus':menus,'lead':lead,'title_ids':titles,'info':info}) 
                    
          return request.website._render("website_capitalcentric.menu_and_covers", {'menus': menus,'lead':lead,'title_ids':titles,'info':info})

      
# Invoice link in Invoice Table
      @http.route(['/website_capitalcentric/invoice_json/'], type='json', auth="public", website=True)
      def invoice_json(self,invoice_id,json=None):
          ...
          user_obj = request.registry.get('res.users')
          user = user_obj.browse(cr,uid,uid)         
          return request.website._render("website_capitalcentric.invoice_link_template", {'invoice': acc_inv ,'user':user})


#Fetching Covers from dialog box in Bookings of Tour Operators    
      @http.route(['/website_capitalcentric/menu_covers_save/'], type='json', auth="public", website=True)
...
#                  'login_user': user,
        ...
        return request.website.render("website_capitalcentric.bookings",values)
...
#My Cart link in TO Header
      ...
      def my_cart(self,*arg, **post ):
        ...
        if user.name == 'Public user':
           ...
           if user.role in ('client_user','client_mng','client_cust'):
              operator = user.partner_id and user.partner_id.parent_id and user.partner_id.parent_id.id or False
           lead_ids = lead_obj.search(cr, uid, [('state','=','draft'),('id','in',request.httprequest.session.get('website_lead_id') or [])])
        ...
        return request.website.render("website_capitalcentric.booking_completion",values)
...
#                  'login_user' : user,
        ...
        return request.website.render("website_capitalcentric.markup_template",values)
...
#                  'login_user'  : user,
        ...
        return request.website.render("website_capitalcentric.invoice",values)
...
#PAYMENTS link in Tour Operators
      ...
      def payments(self,*arg, **post ):  
          ...
          return request.website.render("website_capitalcentric.payments",values)
...
#                      'login_user' : user,
          ...
          return request.website.render("website_capitalcentric.help_form",values)
...
#CONTACTS LINK in Venues & Tour Operators
      ...
      def contacts(self,path,*arg, **post ):
          ...
          return request.website.render("website_capitalcentric.con_info",values)
...
#                     'login_user'   : user,
          ...
          return request.website.render("website_capitalcentric.res_info",values)
...
#                     'login_user' : user,
          ...
          return request.website.render("website_capitalcentric.home_privacy",values)
...
#TC LINK in Footer
      ...
      def terms_footer(self,path,*arg, **post ):
          ...
          return request.website.render("website_capitalcentric.home_tc",{'user':user,'path':path})
...
#Privacy LINK in Footer 
      ...
      def privacy_policy_footer(self,*arg, **post ):
          ...
          return request.website.render("website_capitalcentric.home_privacy",{'user':user,'path':'home_page'})
...
#Contact Us LINK in Footer
      ...
      def contact_us(self, path = None,*arg, **post ):
          ...
          return request.website.render("website_capitalcentric.contact_us",values)
...
#About Us LINK in Footer
      ...
      def about_us(self,*arg, **post ):
          ...
          return request.website.render("website_capitalcentric.about_us",{'user':user})

#Contact Us Fetch Form
      @http.route(['/website_capitalcentric/contact_us_form/','/website_capitalcentric/contact_us_form/<path>'], type='http', auth="public", website=True, multilang=True)
      def contact_us_form(self, path=None, *arg, **post ):
          ...
          if 'g-recaptcha-response' in post:
             ...
             if not verify_rs.get("success", False):
                 post.update({'captcha_error':'Please wait for the reCaptcha image to load and try again.',
                              'user':user})
                 return request.website.render("website_capitalcentric.contact_us",post)
          ...
          body = """<div style='font-family:Calibri; font-size: 12pt; color: rgb(34, 34, 34); background-color: rgb(255, 255, 255); '>
                    """ + str(not user.is_wlabeling and '<img src="https://fitforpurposeonline.com/website_capitalcentric/static/src/img/fitcclogo.jpg" height="156" width="245">' or '') + """
...
#TC SAVED(JSON)       
      @http.route(['/website_capitalcentric/tc_json/'], type='json', auth="public", website=True, multilang=True)
...
#                    'login_user': user,
          ...
          return request.website.render("website_capitalcentric.profile_form",values)
      
       
#FETCHING THE STATE NAMES AND IDS(JSON)
      @http.route(['/website_capitalcentric/state_json/'], type='json', auth="public", website=True, multilang=True)
...
#FETCHING THE Restaurants and Menus IDS(JSON) in Markup Operator
      @http.route(['/website_capitalcentric/markup_json/'], type='json', auth="public", website=True, multilang=True)
...
#FETCHING THE LOCATION NAMES AND IDS(JSON)
      @http.route(['/website_capitalcentric/location_json/'], type='json', auth="public", website=True)
...
#FETCHING THE CITY NAMES AND IDS(JSON)
      @http.route(['/website_capitalcentric/city_json/'], type='json', auth="public", website=True)
      ...
      @http.route(['/website_capitalcentric/address_json/'], type='json', auth="public", website=True)
...
#             street1 = ' '
      ...
      @http.route(['/website_capitalcentric/existing_address_json/'], type='json', auth="public", website=True)
      ...
      def contact_save(self,image_name = None, *args, **post ):
          ...
          if img == "": 
              if(post.get('type') == 'restaurant'):
                   return request.redirect('/capital_centric/invoice_details/restaurant?inv_save=yes');
              if(post.get('type') == 'operator'):
                   return request.redirect('/capital_centric/invoice_details/operator?inv_save=yes');
...
#Request change in Price button in Pricing Menu page
      ...
      def request_change_in_price(self, menu_id=None, **post ):
          ...
          if menu:
              ...
              body = """<div style="font-family:Calibri; font-size: 12pt; color: rgb(34, 34, 34); background-color: rgb(255, 255, 255); "> 
                        <img src="https://fitforpurposeonline.com/website_capitalcentric/static/src/img/fitcclogo.jpg" height="156" width="245">
                        <br/><br/>
      ...
      def adding_contact(self, path, *arg, **post ):
          ...
          return request.website.render("website_capitalcentric.add_contact",values)
...
#                     'login_user' : user,
          ...
          return request.website.render("website_capitalcentric.edit_contact",values)  
      ...
      def edit_restaurant(self, path, child_id=None, **post ):
          ...
          return request.website.render("website_capitalcentric.restaurant_details",values)
       
      
      @http.route(['/website_capitalcentric/pricing_json/'], type='json', auth="public", website=True)
      ...
      def add_restaurant(self, *arg, **post ):
          ...
          return request.website.render("website_capitalcentric.add_restaurant",values)
      
#For Time And Allocations Tab
      @http.route(['/website_capitalcentric/time/'], type='json', auth="public", website=True)
...
#For Time And Allocations Tab Events Menu Sub tab
      @http.route(['/website_capitalcentric/events_menu/'], type='json', auth="public", website=True)
...
#Apply all() for time&allocations
      @http.route(['/website_capitalcentric/existing_alloc_record/'], type='json', auth="public", website=True)
      def existing_alloc_record(self , value, type, db_id, partner_id, name, non_frm_id, non_to_id, std_frm_id, std_to_id, pre_frm_id, pre_to_id, covers, cap, json=None):
          cr, uid, suid, ctx = request.cr, request.uid, openerp.SUPERUSER_ID, request.context
...
#Save() for all one2many objects
      @http.route(['/website_capitalcentric/save_time_rec/'], type='json', auth="public", website=True)
      def save_time_rec(self , dict_time, type, venue_id,create_count, chkwhich, json=None):
          ...
          for m in main_ids:
              ...
              if not vals:                 
                 continue
              del dict_time[str(m)]
              if chkwhich in ('drinks','events_menu') or user.is_group:
                 context = {'mu_update':False}       
                 if vals['sc_included'] == 'False':
                    ...
                    vals['sc_disc'] = False 
                 vals['markup'] = main_obj.get_tourop_price(cr, uid, [], vals['food_type'],float(vals['rest_price']), 0, float(vals['service_charge']), vals['sc_included'], vals.get('sc_disc',False), context)['value']['markup']
      ...
      @http.route(['/website_capitalcentric/existing_menu_record/'], type='json', auth="public", website=True)
...
#                       'login_user' : user,
          ...
          return request.website.render("website_capitalcentric.menu_individual",values)
      
#BLACKOUT TAB
      @http.route(['/website_capitalcentric/blackout_json/'], type='json', auth="public", website=True)
...
#For Deletion Of records in Contacts & Restaurants,Time & Allocations,Menu,Black-Out
      @http.route(['/website_capitalcentric/delete_records/'], type='json', auth="public", website=True)
...
#       Action : onclick logo go to home page 
      ...
      def westfield_search(self,path, *arg, **post ):
          cr, uid, suid, ctx = request.cr, request.uid, openerp.SUPERUSER_ID, request.context
          request.context.update({'editable':False})
          loginuid = request.session.uid or request.uid
          if post is None:post={}  
          ...
          return request.website.render("website_capitalcentric.westfield_search",values)
      ...
      @http.route(['/website_capitalcentric/json_maps_search'], type='json', auth="public", multilang=True, website=True)
      def json_maps_search(self,center_pt,cuisine,dining,min_p,max_p,fav_maps,json=None):
          ...
          if user.role in ('client_cust','client_user'):
             operator = user.partner_id.parent_id                       
             adtinal_domain = [('id','not in',[x.id for x in operator.contact_ids or []])]
      ...
      def search(self,*arg, **post ):
          ...
          return request.website.render("website_capitalcentric.google_maps",values)     
...
#                    'login_user' : user,
        ...
        return request.website.render("website_capitalcentric.result",values)
      ...
      def result_menu(self,path, venue_id=None, **post ):
            ...
            for m in venue.fit_menu_ids:
                if m.food_type == 'meal':
                    ...
                    if m.afternoon_tea:
                       type_list.add('at')
                    if m.dinner:
                       type_list.add('dinner')
                   
        values['meal_tp'] = list(type_list)
        
        for mt in values['meal_tp']:
            cr.execute("select sum(covers) from cc_time_allocations where partner_id = " + str(venue.id) +" and type='" + str(mt == 'bf' and 'break_fast' or mt) +"'")
            covers = cr.fetchone()
            if covers and covers[0] in (0,None):
               values['meal_tp'].remove(mt)
        ...
        if path == 'events' or user.is_group:                  
           ...
           if path == 'events':     
              ...
              if event.covers <= 0:
                 return request.website.render("website_capitalcentric.single_record",values)
           else:
 ...
 #Check for Blackout Day
      @http.route(['/website_capitalcentric/blackout_validation/'], type='json', auth="public", website=True, multilang=True)
      ...
      # Onchange of  guests  
      @http.route(['/website_capitalcentric/cc_menus_table/'], type='json', auth="public", website=True, multilang=True)
      def cc_menus_table(self, guest, db_id, price, menu_ids, frmwch=None, e_date=None, json=None):
          ...
          if frmwch in ('1bf','2lunch','3at','4dinner','events') and not user.is_group:      
             ...
             return request.website._render("website_capitalcentric.events_table",{'event_menus' : menus,
...
#                  operator = user.partner_id.parent_id or False
              ...
              drinks = request.website._render("website_capitalcentric.drinks_table_template",{'drinks' : menus_obj.browse(cr, uid, drink_ids),
                                                                                      'max_covers':venue.max_covers,
                                                                                      'user':user})
              return request.website._render("website_capitalcentric.events_table",{'event_menus' : menus_obj.browse(cr, uid, menu_ids),
              ...
              menu = menus_obj.browse(cr,uid,db_id)
              p_price = menu.pp_person + menu.ffp_mu
              ...
              menus_obj.write(cr, uid, [db_id], menus_vals)
    
              total = 0
              for m in menus_obj.browse(cr, suid, menu_ids):                                               
                  total += m.price
...
# 5 Image Saving functions in Venues...      
      @http.route(['/website_capitalcentric/image/'], type='http', auth='user', methods=['POST'], website=True)
      ...
      @http.route(['/website_capitalcentric/image_desc/'], type='json', auth="public", multilang=True, website=True)
      ...
      @http.route(['/website_capitalcentric/on_change_type/'], type='json', auth="public", website=True, multilang=True)
      ...
      @http.route(['/website_capitalcentric/onclick_date/'], type='json', auth="public", website=True, multilang=True)
      def date_selected_on_calview(self, price_type, type, partner_id, date, min_time, max_time, menu_ids, json=None):
            ...
            return menu_data, time_rec, request.website._render("website_capitalcentric.drinks_table_template", {'drinks': drinks and menutable_obj.browse(cr, suid, drinks) or False,'user':user}), venue.max_covers, wcolor,request.website._render("website_capitalcentric.popover_content", {'times_list': time_rec,'avlblty_dict':avlblty_dict})
      ...
      def menu_select_proceed(self, venue_id, req_time, menu_ids, type=None, event_id=None, json=None):
        ...
        for m in  menutable_obj.browse(cr,uid,menu_ids):            
            day = datetime.strptime(m.booking_date, "%Y-%m-%d").strftime("%A").lower()
            booking_date = m.booking_date
            if m.food_type != 'drink':covers += m.guest 
            if req_time:
               sqlstr = ''
               if m.price_type in ('bf_non_premium', 'l_non_premium', 'at_non_premium', 'd_non_premium'):
                  sqlstr = " and '" + str(req_time) + "' between (select name from cc_time where id = non_frm_id)  and (select name from cc_time where id = non_to_id)"
                  frm_tm = 'non_frm_id'
                  to_tm  = 'non_to_id'
               elif m.price_type in ('bf_standard', 'l_standard', 'at_standard', 'd_standard'):
                    sqlstr = " and '" + str(req_time) + "' between (select name from cc_time where id = std_frm_id)  and (select name from cc_time where id = std_to_id)"
                    frm_tm = 'std_frm_id'
                    to_tm  = 'std_to_id'
               elif m.price_type in ('bf_premium', 'l_premium', 'at_premium', 'd_premium'):
                    sqlstr = " and '" + str(req_time) + "' between (select name from cc_time where id = pre_frm_id)  and (select name from cc_time where id = pre_to_id)"
                    frm_tm = 'pre_frm_id'
                    to_tm  = 'pre_to_id'
        ...
        return request.website._render("website_capitalcentric.booking_completion",values) 
  
#On Click Of "Complete Booking" Button creation of lead,invoice and proceed to payments  
      @http.route(['/website_capitalcentric/guest_details/'], type='http', auth="public", website=True ,multilang=True)
      def complete_booking(self , *args, **post):
          ...
          if post.get('from_wch') == 'complete':
             ...
             return request.website.render("website_capitalcentric.thnk_u_pg",{'response':'for_booking','user':user,'uid':False}) 
     
      @http.route(['/website_capitalcentric/payment_status/'], type='http', auth="public", website=True ,multilang=True)
      def worldpay_response(self , *args, **post):
          ...
          red_url = 'https://' + h +'/capital_centric/fit/grid_view'
          red2_url= 'https://' + h +'/capital_centric/operator/bookings'
          img_url = 'https://' + h
          if context is None: context = {}
            
          is_wlabel = False 
          if h not in ('fitforpurposeonline.com','cc.trabacus.com'):
             is_wlabel = True
...
#           _logger.error('WORLD PAY USWER: %s',user)
          ...
          return request.website.render("website_capitalcentric.thank_you_page",values)
      
      @http.route(['/website_capitalcentric/Thankyou/'], type='http', auth="public", website=True ,multilang=True)
...
#           _logger.error('WORLD PAY FORM: %s', request.website._render("website_capitalcentric.thank_you_page",{})) 
          return request.website.render("website_capitalcentric.thank_you_page",{'user':user})
...
<p></p>
      ...
      @http.route(['/website_capitalcentric/markup_pricing/'], type='json', auth="public", website=True)
      def markup_pricing(self,db_id,restaurant_id,menu_id,partner_id, json=None):
          ...
          if db_id == None:
             try: 
                 ...
                 cr.rollback()
                 return request.website._render("website_capitalcentric.markup_pricing_template", {'error':"You Cannot select same Menu twice."})
                    
          return request.website._render("website_capitalcentric.markup_pricing_template", {'markup': markup_id and markup_obj.browse(cr, uid, markup_id) or False,})
...
# Saving Guest details entered on cart view      
      @http.route(['/website_capitalcentric/save_input_my_cart/'], type='json', auth="public", website=True)
...
#Star Click on Search Page...
      ...
      # Saving Guest details entered on cart view      
      @http.route(['/website_capitalcentric/session_info/'], type='json', auth="public", website=True)
...
#              return  h 
      ...
      def choicedining_banner(self ,*args, **post):
          ...
          return request.website.render("website_capitalcentric.choicedining_banner",values)       
      ...
      def choicedining_groups_menu(self, path, *arg, **post ):
          ...
          return request.website.render("website_capitalcentric.groups_menu",values)
      
      
      # 'Enquiry' Button in Choice Dining    
      @http.route(['/website_capitalcentric/choice_dining_enquiry/'], type='json', auth="public", website=True)
      def choice_dining_enquiry(self,id,json=None):
          ...
          return request.website._render("website_capitalcentric.choice_dining_details", values)
...
class Api(http.Controller): 
    ...
    def get_masters(self , path,*args, **post):
        ...
        try:            
            ...
            master_allowed = {'country':'res.country','state':'res.country.state','title':'res.partner.title',
                              ...
                              'work_detail':'work.detail','restaurants':'res.partner'}
    ...
    def get_available_restaurants(self, path, *args, **post):
        ...
        covers = 0
        try:       
           if post['token'] and post['booking_date'] and post['booking_time']:
              book_date = datetime.strptime(post.get('booking_date'), "%d-%m-%Y")
              book_time = time.strptime(post['booking_time'], '%H:%M')
        ...
        try: 
            ...
            sqlstr = """select  distinct(rp.id) as restaurant_id
                                 ,rp.name as restaurant_name
                                 ,ta.type
                                 ,(case when %s between ta.non_frm_id and ta.non_to_id then 'non_premium'
                                   else case when %s between ta.std_frm_id and ta.std_to_id then 'standard'
                                   else case when %s between ta.pre_frm_id and ta.pre_to_id then 'premium' 
                                   else '' end end end) as meal_type
                                 ,(case when %s between ta.non_frm_id and ta.non_to_id then array_to_string(array(select name 
                                                                                                                 from cc_time 
                                                                                                                 where id between ta.non_frm_id and ta.non_to_id 
                                                                                                                 and id not in (%s , ta.non_to_id)),',')
                                 else case when %s between ta.std_frm_id and ta.std_to_id then array_to_string(array(select name 
                                                                                                                     from cc_time 
                                                                                                                     where id between ta.std_frm_id 
                                                                                                                     and ta.std_to_id and id not in (%s , ta.std_to_id)),',')
                                 else case when %s between ta.pre_frm_id and ta.pre_to_id then array_to_string(array(select name 
                                                                                                                     from cc_time 
                                                                                                                     where id between ta.pre_frm_id and ta.pre_to_id and id not in (%s , ta.pre_to_id)),',')
                                 else '' end end end) || ',' as available_times
                          from res_partner rp
                          inner join cc_time_allocations ta on ta.partner_id = rp.id
                          left outer join cuisine_part_rel c on c.partner_id = rp.id
                          left outer join dining_part_rel d on d.partner_id = rp.id
                          where rp.type='default' 
                          and rp.active=True 
                          and rp.partner_type = 'venue'
                          and ta.name = '%s'
                          and ( %s between ta.non_frm_id and ta.non_to_id-1 or 
                                %s between ta.std_frm_id and ta.std_to_id-1 or 
                                %s between ta.pre_frm_id and ta.pre_to_id-1)
                          and (select (case when sum(covers) + %s is null then 0 else sum(covers) end) as covers 
                               from crm_lead 
                               where conv_dob = '%s' 
                               and conv_tob = '%s' 
                               and restaurant_id = rp.id) < ta.covers                      
                          and rp.id not in (select partner_id 
                                            from cc_black_out_day 
                                            where '%s' between date and date_to 
                                            and %s between from_time_id and to_time_id)
                      """%(time_id, time_id, time_id,
                           time_id, time_id, time_id, time_id, time_id, time_id,
                           day, time_id, time_id, time_id, covers,
                           bking_date, post.get('booking_time'), date.strftime('%Y-%m-%d'), time_id)
            
            if post.get('name'):
               sqlstr += """ and rp.name ilike '%""" + (post.get('name')) + """%'"""
            if post.get('address'):
               sqlstr += """ and (street ilike '%""" + post.get('address') + """%' or 
                             street2 ilike '%""" + post.get('address') + """%' or 
                             city ilike '%""" + post.get('address') + """%' or
                             rp.state_id in (select id from res_country_state where name ilike '%""" + post.get('address') + """%') or 
                             rp.country_id in (select id from res_country where name ilike '%""" + post.get('address') + """%')) """
            
            if post.get('post_code'):
                 sqlstr += """ and rp.zip ilike '""" + str(post.get('post_code')) + """%'"""
            
            if post.get('tube_station'):
                 sqlstr += """ and rp.tstation_id = """ + str(post.get('tube_station')) 
            ...
            if post.get('cuisine'):              
               ...
               if isinstance(eval(cuisine),int):
                  ...
                  sqlstr += """ and c.cuisine_id in """ + str(tuple(eval(cuisine)))
           
           
            if post.get('dining_style'):
               dining = post.get('dining_style')
               if isinstance(eval(dining),int):
                  sqlstr += """ and d.dining_id = """ + str(dining)
               else:    
                  sqlstr += """ and d.dining_id in """ + str(tuple(eval(dining)))
    
            if path == 'confirm':                        
               sqlstr += """ and rp.id = %s"""%(post.get('restaurant_id'))     
    
            sqlstr += """ order by rp.id"""
            ...
            for r in result:
                ...
                if   r['type'] == 'break_fast': meal_type='bf_' + r['meal_type']
                elif r['type'] == 'lunch'     : meal_type='l_' + r['meal_type']
                elif r['type'] == 'at'        : meal_type='at_' + r['meal_type']
                elif r['type'] == 'dinner'    : meal_type='d_' + r['meal_type']
                 
                menus = post.get('menus') or menu_obj.search_read([('partner_id','=',r['restaurant_id']),('food_type','=','meal'),type[r['type']]],['id','name','description','sc_included'])
                if path == 'search':
                   drinks = menu_obj.search_read([('partner_id','=',r['restaurant_id']),('food_type','=','drink')],['id','name','description','to_price','drink_size'])
                   for y in drinks:
                       y['drink_id'] = y.pop('id')
                       y['price'] = y.pop('to_price')                        
                else:
                   drinks = post.get('drinks',[]) 
                   
                for m in menus:
                    if path == 'search':
                       m['menu_id'] = m.pop('id')
                    menu = menu_obj.browse(m['menu_id'])
                    if not menu or menu.partner_id.id != r['restaurant_id']:
                       return self.handle_ffp_Exception('not_found',{'error':'Invalid menu ID','results':[]})
                    
                    for pr in menu.pricing_ids:                    
                        if meal_type == pr.meal_type:                           
                           price,ffp_mu,ffp_co,to_mu,sc = get_price(cr,day,pr,0)
                           if path == 'confirm':
                              tot_price = round((price + ffp_mu + sc),2)
                              if m['price'] != tot_price:
                                 return self.handle_ffp_Exception('reset_content',{'error':'There is change in Menu price.Please update menu with new price and Try Again',
                                                                                   'result':[{'menu_id':menu.id,
                                                                                              'old_price':m['price'],
                                                                                              'new_price':tot_price}] })   
                              m.update({'rp_menu':price,
                                        'top_mp_markup': ffp_mu,
                                        'rp_service_charge' : sc,
                                        'top_service_charge' : sc,
                                        'comsion_pprsn' : ffp_co,
                                        'menu_desc':menu.description
                                         }) 
                                                         
                           m.update({'price':"%.2f" %(price + ffp_mu + sc)}) 
                 
                r.update({'menus':menus,'drinks':drinks})
             
            if path == 'confirm':
               for d in drinks:                   
                    drink = menu_obj.browse(d['id'])
                    d['menu_id'] = d.pop('id')
                    d.update({'rp_drinks':drink.rest_price,
                              'top_drinks_markup': drink.markup,
                              'rp_service_charge': drink.to_price - (drink.rest_price + drink.markup),
                              'top_service_charge' : drink.to_price - (drink.rest_price + drink.markup)})  
                    if d['to_price'] != drink.to_price:
                       return self.handle_ffp_Exception('reset_content',{'error':'There is change in Drink price.Please update drinks with new price and Try Again',
                                                                         'result':[{'drinks_id':drink.id,
                                                                                    'old_price':d['to_price'],
                                                                                    'new_price':drink.to_price}]})
    ...
    def confirm_booking(self, cr ,user, date, post, covers, result): 
        ...
        lead_obj = request.registry.get("crm.lead")
        invoice_obj = request.registry.get("account.invoice") 
        venue = rest_obj.browse(result.get('restaurant_id'))  
        ...
        due_date =  datetime.strptime((parser.parse(''.join((re.compile('\d')).findall(date.strftime('%Y-%m-%d'))))).strftime('%Y-%m-30'), "%Y-%m-%d") + relativedelta(months=1)
        invoice_obj.write(cr,user.id,invoice_ids,{'date_invoice':inv_date,'date_due':due_date})  
        vtemplate_id   = request.registry.get("email.template").search(cr,user.id,[('name','=','Enquiry - Booking Confirmation To Venue1')])
        try:
...
#   Video Template
    ...
    def video_template(self,*arg, **post ):
        ...
        user_obj = request.registry.get('res.users')
        user = user_obj.browse(cr,uid,uid)
        return request.website.render("website_capitalcentric.video_template",{'user':user})
