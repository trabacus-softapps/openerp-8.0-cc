...
{     
	  ...
	  if(accept_tc == '')
	  {
		  ...
		  $(".button_tc").on("click", function()
		  {
		  	...
		  	openerp.jsonRpc("/website_capitalcentric/tc_json/", 'call', {'partner_id':parseInt(partner_id)})
		   .then(function (data) 
		    {
	  ...
	  if(accept_tc == 'True')
	  {
		  ...
		  $(".button_tc").on("click", function()
		  {
		  	...
		  	openerp.jsonRpc("/website_capitalcentric/tc_json/", 'call', {'cancellation_hrs':cancellation_hrs,'partner_id':parseInt(partner_id)})
		    .then(function (data) 
		    {
			   	 $("input[name='accept_tc']").prop('disabled', true);
	   	  	 	 if( document.getElementById('path').value == 'restaurant')
