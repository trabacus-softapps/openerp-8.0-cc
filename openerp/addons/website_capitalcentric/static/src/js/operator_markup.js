...
// Adding new Records for Restaurant Level & Menu Level Exceptions Table
    ...
    {	
        ...
        if(id_name == 'res_l_add')
        {
	    	...
	    	"<td class='singleblock_text' style='padding-top:1%;padding-bottom:1%;padding-right:1%;'><select id='res_select_ex' class='input-control restaurant_select' style='color:black;padding-left:5px;width:100%;height:24px;'><option value=''> Select a Restaurant </option></select></td>"+
	    	"<td class='singleblock_number' style='padding-top:1%;padding-bottom:1%;padding-left:1%;'><input type='text' id='' class='input-control res_mrkup_per' style='color:black;padding-right:5px;text-align:right;width:100%;height:24px;'></input></td>"+
	    	"<td class='singleblock_center' style='padding-top:1%;padding-bottom:1%;'><a id='db_delete' class='bookinginfolink'><img title='' src='/website_capitalcentric/static/src/img/delete_t.png' style='width:18px;height:18px;'/></a></td>"+
	    	"</tr>";
	    	
	    	$(newRow).appendTo($("#res_ex_table"));
    		$("#res_ex_table tr td select").focus();
    		
    		openerp.jsonRpc("/website_capitalcentric/markup_json/", 'call', {'from_which':'restaurant','restaurant_id':' '})
	        .then(function (data) 
	        {	                
        ...
        else
        {
        	...
        	var newRow ="<tr>"+
	    	"<td class='hidden'><input id='db_id' value='undefined'/></td>"+
	    	"<td class='singleblock_text' style='padding-top:1%;padding-bottom:1%;padding-right:1%;'><select id='res_select_ex' class='input-control restaurant_select new_record' onchange='javascript:change_rest(this)' style='color:black;padding-left:5px;width:100%;height:24px;'><option value=''> Select a Restaurant </option></select></td>"+
	    	'<td class="singleblock_text"><select disabled="disabled" multiple="multiple" name="multiple_select_name" id="'+ (parseInt(x)+1) +'" class="input-control input_val multiple_menu" title="" onchange="javascript:get_menusvalues(this)"></select><textarea class="hidden" name="menus_selected" id="menu_ids"></textarea></td>'+
	    	"<td class='singleblock_number' style='padding-top:1%;padding-bottom:1%;padding-left:1%;'><input type='text' id='' class='input-control menu_mrkup_per' style='color:black;padding-right:5px;text-align:right;width:100%;height:24px;'></input></td>"+
	    	"<td class='singleblock_center' style='padding-top:1%;padding-bottom:1%;'><a id='db_delete' class='bookinginfolink'><img title='' src='/website_capitalcentric/static/src/img/delete_t.png' style='width:18px;height:18px;'/></a></td>"+
	    	"</tr>";
...
//Deletion of Records for Restaurant Level & Menu Level Exceptions Table
    $(".common_del").on('click','#db_delete',function(event)
    {
	    ...
	    else
	    {
	        ...
	        if (r==true)
	        {
			    openerp.jsonRpc("/website_capitalcentric/delete_records/", 'call', {'value' : 'markup_table', 'db_id' : parseInt(db_id)})
		        .then(function (data) 
		        {
		         	id.remove();
		         	document.body.scrollTop = document.documentElement.scrollTop = 0;
...
// Save for Markup Table
	$(".save_markup").click(function()
    {
		...
		{	
			...
			openerp.jsonRpc("/website_capitalcentric/save_time_rec/", 'call', {'dict_time':vals,'type':'','venue_id':parseFloat(markup_perc),'create_count':count,'chkwhich':'markup'})
		  	.then(function (data)
...
// // Adding a new markup
    ...
    // {	
    	...
    	// var newRow ="<tr>"+
    	// "<td class='hidden'><input id='db_id' value='undefined'/></td>"+
    	// "<td class='singleblock_text' style='padding-top:1%;padding-bottom:1%;'><select title='' class='input-control' onchange='javascript:partner_onchange(this)' style='color:black;padding-left:0.5%;width:100%;height:24px;' id='restaurant_id' name='restaurant_id'><option value=''> Select a Restaurant </option></select></td>"+
    	// "<td class='singleblock_text' style='padding-top:1%;padding-bottom:1%;'><select required='required' title='' class='input-control' style='color:black;padding-left:0.5%;width:100%;height:24px;' id='menu_id' name='menu_id'></select></td>"+
    	...
    	// "<td class='singleblock_center' style='padding-top:1%;padding-bottom:1%;'><a id='db_delete' class='bookinginfolink'><img title='' src='/website_capitalcentric/static/src/img/delete_t.png' style='width:18px;height:18px;' id='img_icons'/></a></td>"+
    	// "</tr>";
//     	
...
// //Add Markup Button in markup Table
    ...
    // {	 
    	 ...
    	 // {
    	 	...
    	 	// return false;
    	 // }
    	 // openerp.jsonRpc("/website_capitalcentric/markup_pricing/", 'call', {'partner_id':parseInt($("#partner_id").val()),'restaurant_id' : parseInt(restaurant_id),'menu_id' : parseInt(menu_id), 'db_id' : parseInt(db_id)})
        // .then(function (data) 
        // {
...
// 	        
	        // for (i=0;i < m.length; i++)
	 		...
	 		// } 	
// 
	 		// openerp.jsonRpc("/website_capitalcentric/pricing_json/", 'call', {'dict_pricing':vals,'menu_id':parseInt(markup_id), 'chkwhich':'m_save','sc_included':'','service_charge':'', 'sc_disc':''})
		  	// .then(function (data)
		  	// {	
...
// 	        
		    ...
		    // }
		    // console.log(dict);
		    // openerp.jsonRpc("/website_capitalcentric/pricing_json/", 'call', {'dict_pricing':dict,'menu_id':parseInt(markup), 'chkwhich':'m_apply_all','sc_included':'','service_charge':'', 'sc_disc':''})
		   // .then(function (data)
		   // { 
...
{	
	...
	var ul = '';
	openerp.jsonRpc("/website_capitalcentric/markup_json/", 'call', {'from_which':'menu','restaurant_id':parseInt(e.value)})
    .then(function (data) 
    {      	
    	 if(data[0].length > 0)
    	 {	 	 
