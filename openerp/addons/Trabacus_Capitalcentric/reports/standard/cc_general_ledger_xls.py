...
class general_ledger_xls(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        c_specs = [
            ...
            ('source', 1, 0, 'text', _('Source'), None, c_hdr_cell_style),
            ('status', 1, 0, 'text', _('Status'), None, c_hdr_cell_style),
            ('account_code', 1, 0, 'text', _('Account Code'), None, c_hdr_cell_style),
            ('account_name', 1, 0, 'text', _('Account Name'), None, c_hdr_cell_style),
            ...
            ('line_bal', 1, 0, 'text', _('Amount'), None, c_hdr_cell_style_right),
        ...
        for account in objects:
            ...
            if _p.display_account_raw(data) == 'all' or (display_ledger_lines or display_initial_balance):
                ...
                if display_initial_balance:
                    ...
                    line_balance = cumul_debit - cumul_credit
                    print 'line_balance',line_balance
                    ...
                    c_specs = [('empty%s' %x, 1, 0, 'text', None) for x in range(4)]
                    c_specs += [
                        ('source', 1, 0, 'text', 'FFP'),
                        ('status', 1, 0, 'text', None),
                        ('account_code', 1, 0, 'text', account.code),
                        ('account_name', 1, 0, 'text', account.name),
                        ('partner', 1, 0, 'text', None),
                        ...
                        ('line_bal', 1, 0, 'number', line_balance, None, c_init_cell_style_decimal),
                ...
                for line in account.ledger_lines:
                    ...
                    line_balance = (line.get('debit') or 0.0) - (line.get('credit') or 0.0)
                    print 'line_balance',line_balance
                    ...
                    c_specs += [
                        ('period', 1, 0, 'text', datetime.strptime(line.get('period_code'),'%m/%Y').strftime('%b %y') or ''),
                        ('move', 1, 0, 'text', line.get('move_name') or ''),
                        ('journal', 1, 0, 'text', line.get('jcode') or ''),
                        ('source', 1, 0, 'text', 'FFP'),
                        ('status', 1, 0, 'text', line.get('state') or ''),
                        ('account_code', 1, 0, 'text', account.code),
                        ('account_name', 1, 0, 'text', account.name),
                        ...
                        ('line_bal', 1, 0, 'number', line_balance, None, ll_cell_style_decimal),
...
general_ledger_xls('report.account.cc_account_report_general_ledger_xls', 'account.account',
