...
class third_party_ledger_cc(report_sxw.rml_parse, crh.common_report_header):

    def __init__(self, cr, uid, name, context=None):
        super(third_party_ledger_cc, self).__init__(cr, uid, name, context=context)
    ...
    def _get_filter(self, data):
        ...
        return super(third_party_ledger_cc, self)._get_filter(data)
...
#                    "AND " + self.query +" " \
        ...
        if data['form']['partner_id']==False:
           self.partner_ids = [res['partner_id'] for res in self.cr.dictfetchall()]
        else:
            self.partner_ids = [data['form']['partner_id'][0]]        
        objects = obj_partner.browse(self.cr, self.uid, self.partner_ids)
        return super(third_party_ledger_cc, self).set_context(objects, data, self.partner_ids, report_type)
...
report_sxw.report_sxw('report.account.third_party_ledger_cc', 'res.partner',
        'addons/Trabacus_Capitalcentric/reports/standard/cc_partner_ledger.rml',parser=third_party_ledger_cc,
        header='internal')

report_sxw.report_sxw('report.account.third_party_ledger_other_cc', 'res.partner',
        'addons/Trabacus_Capitalcentric/reports/standard/cc_partner_ledger_other.rml',parser=third_party_ledger_cc,
        header='internal')
